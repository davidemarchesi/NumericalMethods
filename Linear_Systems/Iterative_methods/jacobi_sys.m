function[xn,k,err_acc]=jacobi_sys(A,b,x0,tol,nmax)
% Jacobi resolutive method for Linear systems
% 
% Inputs:
% A = System matrix;
% b = known terms;
...
% to use the jacobi method a squared matrix 'nxn' invertible (so with 
...
elseif (prod(diag(A)) == 0)
  error('The matrix A is not invertible!')
    ...
    while (err>tol && k<nmax)
        for i = 1:n     
            xn(i) = (b(i) - A(i,1:i-1)*xv(1:i-1) - A(i,i+1:n)*xv(i+1:n)) / A(i,i);
        end
        k = k+1;
        r = b - A*xn;
        err = norm(r)/norm(b);
